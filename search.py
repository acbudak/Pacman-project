...
def depthFirstSearch(problem: SearchProblem):
    ...
    currentPath=Stack()
    stack.pop()
    while problem.isGoalState(current):
        if current not in checked:
            checked.append(current)
            successors = problem.getSuccessors(current)
            for child,route,cost in successors:
                stack.push(child)
                tmpPath = path + [route]
                currentPath.push(tmpPath)
        current = stack.pop()
        ...
        if current not in checked:
            ...
            for child,route,cost in successors:
                ...
                if child not in checked:
                    priorityQueue.push(child,costLeft)
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not problem.isGoalState(current):
        if current not in checked:
            ...
            for child,route,cost in successors:
                tempPath = path + [route]
                costLeft = problem.getCostOfActions(tempPath) + heuristic(child,problem)
                if child not in checked:
                    priorityQueue.push(child,costLeft)
                    currentPath.push(tempPath,costLeft)
        current = priorityQueue.pop()
